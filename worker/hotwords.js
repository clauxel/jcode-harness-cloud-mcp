...
(()=>{const cfg=window.SITE_CONFIG;const $=(s,r=document)=>r.querySelector(s);const $$=(s,r=document)=>Array.from(r.querySelectorAll(s));let lastCheckoutUrl='';let popupRef=null;function esc(v){return String(v??'').replace(/[&<>"]/g,c=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;'}[c]));}function uuid(){return crypto.randomUUID?crypto.randomUUID():String(Date.now())+Math.random().toString(16).slice(2);}function storageValue(key,session){try{const store=session?sessionStorage:localStorage;let value=store.getItem(key);if(value)return value;value=uuid();store.setItem(key,value);return value}catch{return uuid()}}const visitorId=storageValue(cfg.slug+'_visitor_id');const sessionId=storageValue(cfg.slug+'_session_id',true);function eventPayload(name,metadata){const params=new URLSearchParams(location.search);return{id:uuid(),name,path:location.pathname,visitorId,sessionId,referrerHost:document.referrer?new URL(document.referrer).hostname:'',utmSource:params.get('utm_source')||'',utmMedium:params.get('utm_medium')||'',utmCampaign:params.get('utm_campaign')||'',utmTerm:params.get('utm_term')||'',utmContent:params.get('utm_content')||'',occurredAt:new Date().toISOString(),metadata:metadata||{}}}function track(name,metadata){try{const body=JSON.stringify({events:[eventPayload(name,metadata)]});if(navigator.sendBeacon){navigator.sendBeacon('/api/analytics/events',new Blob([body],{type:'application/json'}));return}fetch('/api/analytics/events',{method:'POST',headers:{'Content-Type':'application/json'},body,keepalive:true}).catch(()=>{})}catch{}}function ensurePaymentUi(){if(!$('#payment-overlay')){const overlay=document.createElement('div');overlay.className='payment-overlay';overlay.id='payment-overlay';overlay.setAttribute('aria-live','polite');overlay.innerHTML='<div class="payment-card"><span class="eyebrow">Hosted payment</span><h2>Opening NOWPayments checkout</h2><p class="section-lede">Keep this page open. The invoice opens in a centered payment window while the product page stays blurred behind it.</p><div class="hero-actions"><button class="btn btn-primary" id="payment-link-fallback" type="button">Open payment window</button><button class="btn" id="payment-close" type="button">Keep browsing</button></div></div>';document.body.appendChild(overlay);$('#payment-close')?.addEventListener('click',()=>setPaymentActive(false));$('#payment-link-fallback')?.addEventListener('click',()=>{if(lastCheckoutUrl)window.open(lastCheckoutUrl,cfg.slug+'Payment',centeredFeatures(760,900));});}if(!$('#paid-notice')){const notice=document.createElement('div');notice.className='notice';notice.id='paid-notice';notice.innerHTML='<strong>Payment required to run this workflow.</strong><p class="muted">Interactive reports, exports, and team history unlock after checkout.</p><div class="hero-actions"><button class="btn btn-primary" data-checkout="'+esc(cfg.plan||'team')+'">Checkout '+esc(cfg.planName||'Team')+' annual</button><a class="btn" href="/pricing/">View plans</a></div>';document.body.appendChild(notice);}}function setPaymentActive(active){ensurePaymentUi();document.body.classList.toggle('payment-active',Boolean(active));}function centeredFeatures(width,height){const w=Math.min(width,Math.max(360,window.screen.availWidth-80));const h=Math.min(height,Math.max(520,window.screen.availHeight-80));const left=Math.max(0,Math.round((window.screen.availWidth-w)/2));const top=Math.max(0,Math.round((window.screen.availHeight-h)/2));return 'popup=yes,width='+w+',height='+h+',left='+left+',top='+top+',noopener=no,noreferrer=no';}function openPopupShell(){popupRef=window.open('',cfg.slug+'Payment',centeredFeatures(760,900));if(popupRef){try{popupRef.document.write('<!doctype html><title>Opening payment...</title><body style="margin:0;background:#07111f;color:#f8fafc;font-family:system-ui;display:grid;place-items:center;min-height:100vh"><main style="max-width:360px;text-align:center"><h1 style="font-size:22px">Opening NOWPayments...</h1><p>Please keep the original '+esc(cfg.name)+' page open.</p></main></body>');popupRef.document.close()}catch{}}return popupRef;}function refresh(){const text=($('#workInput')?.value||cfg.sample||'').trim();const risk=$('#riskLevel')?.value||'standard';const region=$('#region')?.value||'US';const score=Math.max(70,Math.min(98,84+(text.length%13)-(risk==='strict'?4:0)));const scoreEl=$('#scoreValue');if(scoreEl)scoreEl.textContent=score;const list=$('#receiptList');if(list)list.innerHTML=cfg.outputs.map((item,i)=>'<li><span>'+esc(item)+' - '+(i===0?esc(region)+' evidence ready':esc(risk)+' review')+'</span></li>').join('');const summary=$('#previewSummary');if(summary)summary.textContent=text?text.slice(0,150)+(text.length>150?'...':''):'Paste a sample to generate a preview.';}function showPaidNotice(message,source){ensurePaymentUi();const notice=$('#paid-notice');if(!notice)return;const p=notice.querySelector('p');if(p)p.textContent=message||'Generate preview is available after checkout. Choose a plan to unlock interactive reports, exports, and team history.';notice.classList.add('show');track('paid_feature_prompt',{feature:'generate_preview',source:source||'preview_button'});}async function checkout(e){const target=e.currentTarget;if(location.protocol==='file:')return;const plan=target.dataset.checkout||cfg.plan;e.preventDefault();track('checkout_requested',{plan,billing:'annual',source:target.dataset.source||'site'});setPaymentActive(true);const popup=openPopupShell();try{const response=await fetch('/api/checkout',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({planId:plan,billing:'annual',source:'site'})});const payload=await response.json();if(!response.ok||!payload.checkoutUrl)throw new Error(payload.message||payload.error||'Checkout could not be started.');lastCheckoutUrl=payload.checkoutUrl;track('checkout_started',{plan:payload.planId,amountCents:payload.amountCents,currency:payload.currency,orderId:payload.orderId});if(popup&&!popup.closed){popup.location.href=payload.checkoutUrl}else{$('#payment-link-fallback')?.focus();}const checker=setInterval(()=>{if(!popup||popup.closed){clearInterval(checker);setPaymentActive(false);track('checkout_popup_closed',{plan})}},1200)}catch(error){track('checkout_error',{message:String(error.message||error),plan});if(popup&&!popup.closed)popup.close();setPaymentActive(false);const notice=$('#paid-notice');if(notice){const p=notice.querySelector('p');if(p)p.textContent=String(error.message||error)||'Checkout is not available on this deployment yet. Please try again after payment is configured.';notice.classList.add('show')}}}document.addEventListener('DOMContentLoaded',()=>{ensurePaymentUi();const input=$('#workInput');if(input&&!input.value)input.value=cfg.sample||'';$$('[data-refresh]').forEach(b=>b.addEventListener('click',e=>{e.preventDefault();showPaidNotice('Generate preview is available after checkout. Choose a plan to unlock interactive reports, exports, and team history.','generate_preview');}));$$('[data-checkout]').forEach(a=>a.addEventListener('click',checkout));refresh();if(new URLSearchParams(location.search).get('checkout')==='success'||new URLSearchParams(location.search).get('status')==='success'){track('checkout_success_return',{provider:'nowpayments'})}track('page_view',{title:document.title})});})();`
